...
def task_generator(env, scheduler, workload_type, workload_intensity):
    ...
    while True:
        ...
        # Adjust arrival interval based on workload_intensity
...
def run_experiments():
    ...
    for workload_type in ["CPU-bound", "IO-bound", "Mixed"]:
        for workload_intensity in ["Low", "Moderate", "High"]:
            ...
            # Start processes with specified workload type and intensity
...
summary.to_csv("summary_project.csv", index=False)
print("Summary data saved to 'summary_project.csv'")